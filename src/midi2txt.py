...
from utils import pit2str, pos2str, bom2str, dur2str, trk2str, ins2str, pit2alphabet
...
def mp_handler(file_paths, i=None):
    start = time.time()
    ...
    with open(f"data/raw/raw_corpus_{i}.txt", "w", encoding="utf-8") as f:
        for idx, piece in enumerate(event_seq_res):
            f.write(' '.join(piece) + '\n')
    # with open(f"src/brahms.txt", "w", encoding="utf-8") as f:
    #     for idx, piece in enumerate(event_seq_res):
    #         f.write(' '.join(piece) + '\n')
...
if __name__ == '__main__':
    ...
    for i in range(0, len(file_paths), segment):
        mp_handler(file_paths[i:i+segment],i)
